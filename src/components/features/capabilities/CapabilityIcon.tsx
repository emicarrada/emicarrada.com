...
import { CapabilityIconId } from '@/constants/capabilities';

const iconClass = 'h-6 w-6 text-[#FF8200]';

const icons: Record<CapabilityIconId, React.ReactNode> = {
  fullstack: (
...
export function CapabilityIcon({ id, size = 'default' }: CapabilityIconProps) {
  const boxClass = size === 'large' ? 'h-14 w-14' : 'h-11 w-11';

  return (
    <div className={`flex ${boxClass} shrink-0 items-center justify-center rounded-2xl bg-white/[0.04]`}>
      {icons[id]}
    </div>
  );
}
