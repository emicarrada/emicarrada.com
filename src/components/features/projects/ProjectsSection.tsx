import React from 'react';
// @ts-ignore
import ProjectCard from '../../ProjectCard';
import { useProjects } from '../../../hooks/useProjects';

export const ProjectsSection: React.FC = () => {
  const { projects, loading, error } = useProjects();

  if (loading) {
    return (
      <section id="proyectos" className="w-full py-20 bg-[#041737] text-white">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <div className="text-white">Cargando proyectos...</div>
        </div>
      </section>
    );
  }

  if (error) {
    return (
      <section id="proyectos" className="w-full py-20 bg-[#041737] text-white">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <div className="text-red-400">Error: {error}</div>
        </div>
      </section>
    );
  }

  // Separar proyectos por categoría
  const startupProjects = projects.filter(project => project.category === 'startup');
  const scientificProjects = projects.filter(project => project.category === 'scientific');

  // Función para renderizar los tech badges con SVGs inline (diseño original)
  const renderTechBadges = (technologies: string[]) => {
    const techSVGs: { [key: string]: React.ReactElement } = {
      'Firebase': (
        <span title="Firebase" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="https://www.w3.org/2000/svg" width="28" height="28" viewBox="0,0,256,256" style={{fill:'#FFFFFF'}}>
            <g fill="#ffffff" fillRule="nonzero" stroke="none" strokeWidth="1" strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit="10" strokeDasharray="" strokeDashoffset="0" fontFamily="none" fontWeight="none" fontSize="none" textAnchor="none" style={{mixBlendMode: 'normal'}}><g transform="scale(10.66667,10.66667)"><path d="M21.612,17.714l-2.125,-13c-0.059,-0.358 -0.307,-0.657 -0.649,-0.78c-0.342,-0.121 -0.724,-0.05 -0.997,0.189l-3.247,2.845l-1.829,-3.551c-0.166,-0.321 -0.492,-0.528 -0.853,-0.541c-0.362,-0.013 -0.701,0.169 -0.89,0.478l-1.092,1.786l-2.338,-4.619c-0.191,-0.377 -0.603,-0.585 -1.019,-0.54c-0.42,0.054 -0.76,0.366 -0.85,0.78l-3.7,17.026c-0.117,0.539 0.225,1.072 0.765,1.189c0.027,0.006 0.054,0.005 0.081,0.009c0.003,0.001 0.004,0.004 0.007,0.005l8.455,4.875c0.154,0.09 0.326,0.135 0.499,0.135c0.174,0 0.348,-0.045 0.503,-0.136l8.795,-5.125c0.358,-0.209 0.551,-0.616 0.484,-1.025zM11.801,5.915l1.242,2.411l-1.039,0.91l-1.012,-1.998zM10.451,10.597l-3.244,2.842l2.56,-4.193zM7.079,3.937l1.626,3.211l-3.607,5.907zM11.827,21.844l-6.742,-3.887l7.357,-6.446c0.007,-0.008 0.016,-0.012 0.023,-0.02l5.339,-4.677l1.724,10.543z"></path></g></g>
          </svg>
          <span className="ml-2 text-sm">Firebase</span>
        </span>
      ),
      'Node.js': (
        <span title="Node.js" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="https://www.w3.org/2000/svg" width="28" height="28" viewBox="0,0,256,256" style={{fill:'#FFFFFF'}}><g fill="#ffffff" fillRule="nonzero" stroke="none" strokeWidth="1" strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit="10" strokeDasharray="" strokeDashoffset="0" fontFamily="none" fontWeight="none" fontSize="none" textAnchor="none" style={{mixBlendMode: 'normal'}}><g transform="scale(8,8)"><path d="M15.99414,3c-0.365,0 -0.72992,0.08953 -1.04492,0.26953l-9.91016,5.7207c-0.64,0.37 -1.03906,1.07055 -1.03906,1.81055v10.37891c0,0.75 0.39906,1.44055 1.03906,1.81055l2.60156,1.5c1.26,0.62 1.7093,0.61914 2.2793,0.61914c1.87,0 2.93945,-1.12984 2.93945,-3.08984v-10.70898c0,-0.16 -0.12906,-0.29102 -0.28906,-0.29102h-1.25c-0.17,0 -0.29101,0.13102 -0.29101,0.29102v10.69922c0,0.88 -0.90891,1.73977 -2.37891,1.00977l-2.7207,-1.57031c-0.1,-0.05 -0.16016,-0.15953 -0.16016,-0.26953v-10.36914c0,-0.12 0.06016,-0.22125 0.16016,-0.28125l9.91016,-5.71875c0.09,-0.06 0.21055,-0.06 0.31055,0l9.91016,5.71875c0.1,0.06 0.16016,0.16148 0.16016,0.27148v10.37891c0,0.11 -0.06039,0.21953 -0.15039,0.26953l-9.91992,5.73047c-0.09,0.05 -0.22055,0.05 -0.31055,0l-2.55078,-1.50977c-0.07,-0.05 -0.16828,-0.05953 -0.23828,-0.01953c-0.71,0.4 -0.84,0.44969 -1.5,0.67969c-0.16,0.05 -0.41016,0.14969 0.08984,0.42969l3.30859,1.96094c0.32,0.18 0.68102,0.2793 1.04101,0.2793c0.37,0 0.72883,-0.0993 1.04883,-0.2793l9.92188,-5.73047c0.64,-0.37 1.03906,-1.06055 1.03906,-1.81055v-10.36914c0,-0.75 -0.39906,-1.44055 -1.03906,-1.81055l-9.92188,-5.73047c-0.315,-0.18 -0.67992,-0.26953 -1.04492,-0.26953zM18.66016,11.00586c-2.83,0 -4.51953,1.19922 -4.51953,3.19922c0,2.17 1.67844,2.76906 4.39844,3.03906c3.25,0.32 3.5,0.80141 3.5,1.44141c0,1.1 -0.88852,1.57031 -2.97852,1.57031c-2.63,0 -3.21039,-0.66094 -3.40039,-1.96094c-0.02,-0.14 -0.1393,-0.24023 -0.2793,-0.24023h-1.29101c-0.16,0 -0.2793,0.13125 -0.2793,0.28125c0,1.67 0.91,3.6582 5.25,3.6582c3.14,0 4.93945,-1.23844 4.93945,-3.39844c0,-2.14 -1.45023,-2.71109 -4.49023,-3.12109c-3.09,-0.4 -3.40039,-0.61008 -3.40039,-1.33008c0,-0.6 0.27078,-1.38867 2.55078,-1.38867c2.03,0 2.78961,0.43859 3.09961,1.80859c0.03,0.13 0.1393,0.23047 0.2793,0.23047h1.29102c0.08,0 0.14898,-0.03961 0.20898,-0.09961c0.05,-0.05 0.08031,-0.1307 0.07031,-0.2207c-0.2,-2.36 -1.76922,-3.46875 -4.94922,-3.46875z"></path></g></g></svg>
          <span className="ml-2 text-sm">Node.js</span>
        </span>
      ),
      'Express.js': (
        <span title="Express.js" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="https://www.w3.org/2000/svg" viewBox="0 0 32 32" width="28" height="28"><path fill="#fff" d="M32 24.795c-1.164.296-1.884.013-2.53-.957l-4.594-6.356-.664-.88-5.365 7.257c-.613.873-1.256 1.253-2.4.944l6.87-9.222-6.396-8.33c1.1-.214 1.86-.105 2.535.88l4.765 6.435 4.8-6.4c.615-.873 1.276-1.205 2.38-.883l-2.48 3.288-3.36 4.375c-.4.5-.345.842.023 1.325L32 24.795zM.008 15.427l.562-2.764C2.1 7.193 8.37 4.92 12.694 8.3c2.527 1.988 3.155 4.8 3.03 7.95H1.48c-.214 5.67 3.867 9.092 9.07 7.346 1.825-.613 2.9-2.042 3.438-3.83.273-.896.725-1.036 1.567-.78-.43 2.236-1.4 4.104-3.45 5.273-3.063 1.75-7.435 1.184-9.735-1.248C1 21.6.434 19.812.18 17.9c-.04-.316-.12-.617-.18-.92q.008-.776.008-1.552zm1.498-.38h12.872c-.084-4.1-2.637-7.012-6.126-7.037-3.83-.03-6.58 2.813-6.746 7.037z" /></svg>
          <span className="ml-2 text-sm">Express.js</span>
        </span>
      ),
      'React': (
        <span title="React" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg width="28" height="28" viewBox="0 0 569 512" xmlns="https://www.w3.org/2000/svg"><g fill="#fff" fillRule="evenodd"><g fill="#fff" fillRule="nonzero"><g><path d="M285.5,201 C255.400481,201 231,225.400481 231,255.5 C231,285.599519 255.400481,310 285.5,310 C315.599519,310 340,285.599519 340,255.5 C340,225.400481 315.599519,201 285.5,201" /><path d="M568.959856,255.99437 C568.959856,213.207656 529.337802,175.68144 466.251623,150.985214 C467.094645,145.423543 467.85738,139.922107 468.399323,134.521063 C474.621631,73.0415145 459.808523,28.6686204 426.709856,9.5541429 C389.677085,-11.8291748 337.36955,3.69129898 284.479928,46.0162134 C231.590306,3.69129898 179.282771,-11.8291748 142.25,9.5541429 C109.151333,28.6686204 94.3382249,73.0415145 100.560533,134.521063 C101.102476,139.922107 101.845139,145.443621 102.708233,151.02537 C97.4493791,153.033193 92.2908847,155.161486 87.3331099,157.39017 C31.0111824,182.708821 0,217.765415 0,255.99437 C0,298.781084 39.6220545,336.307301 102.708233,361.003527 C101.845139,366.565197 101.102476,372.066633 100.560533,377.467678 C94.3382249,438.947226 109.151333,483.32012 142.25,502.434597 C153.629683,508.887578 166.52439,512.186771 179.603923,511.991836 C210.956328,511.991836 247.567589,495.487529 284.479928,465.972527 C321.372196,495.487529 358.003528,511.991836 389.396077,511.991836 C402.475265,512.183856 415.36922,508.884856 426.75,502.434597 C459.848667,483.32012 474.661775,438.947226 468.439467,377.467678 C467.897524,372.066633 467.134789,366.565197 466.291767,361.003527 C529.377946,336.347457 569,298.761006 569,255.99437 M389.155214,27.1025182 C397.565154,26.899606 405.877839,28.9368502 413.241569,33.0055186 C436.223966,46.2772304 446.540955,82.2775015 441.522965,131.770345 C441.181741,135.143488 440.780302,138.556788 440.298575,141.990165 C414.066922,134.08804 387.205771,128.452154 360.010724,125.144528 C343.525021,103.224055 325.192524,82.7564475 305.214266,63.9661533 C336.586743,39.7116483 366.032313,27.1025182 389.135142,27.1025182 M378.356498,310.205598 C368.204912,327.830733 357.150626,344.919965 345.237759,361.405091 C325.045049,363.479997 304.758818,364.51205 284.459856,364.497299 C264.167589,364.51136 243.888075,363.479308 223.702025,361.405091 C211.820914,344.919381 200.80007,327.83006 190.683646,310.205598 C180.532593,292.629285 171.306974,274.534187 163.044553,255.99437 C171.306974,237.454554 180.532593,219.359455 190.683646,201.783142 C200.784121,184.229367 211.770999,167.201087 223.601665,150.764353 C243.824636,148.63809 264.145559,147.579168 284.479928,147.591877 C304.772146,147.579725 325.051559,148.611772 345.237759,150.68404 C357.109048,167.14607 368.136094,184.201112 378.27621,201.783142 C388.419418,219.363718 397.644825,237.458403 405.915303,255.99437 C397.644825,274.530337 388.419418,292.625022 378.27621,310.205598 M419.724813,290.127366 C426.09516,307.503536 431.324985,325.277083 435.380944,343.334682 C417.779633,348.823635 399.836793,353.149774 381.668372,356.285142 C388.573127,345.871232 395.263781,335.035679 401.740334,323.778483 C408.143291,312.655143 414.144807,301.431411 419.805101,290.207679 M246.363271,390.377981 C258.848032,391.140954 271.593728,391.582675 284.5,391.582675 C297.406272,391.582675 310.232256,391.140954 322.737089,390.377981 C310.880643,404.583418 298.10766,417.997563 284.5,430.534446 C270.921643,417.999548 258.18192,404.585125 246.363271,390.377981 Z M187.311556,356.244986 C169.137286,353.123646 151.187726,348.810918 133.578912,343.334682 C137.618549,325.305649 142.828222,307.559058 149.174827,290.207679 C154.754833,301.431411 160.736278,312.655143 167.239594,323.778483 C173.74291,334.901824 180.467017,345.864539 187.311556,356.285142 M149.174827,221.760984 C142.850954,204.473938 137.654787,186.794745 133.619056,168.834762 C151.18418,163.352378 169.085653,159.013101 187.211197,155.844146 C180.346585,166.224592 173.622478,176.986525 167.139234,188.210257 C160.65599,199.433989 154.734761,210.517173 149.074467,221.760984 M322.616657,121.590681 C310.131896,120.827708 297.3862,120.385987 284.379568,120.385987 C271.479987,120.385987 258.767744,120.787552 246.242839,121.590681 C258.061488,107.383537 270.801211,93.9691137 284.379568,81.4342157 C297.99241,93.9658277 310.765727,107.380324 322.616657,121.590681 Z M401.70019,188.210257 C395.196875,176.939676 388.472767,166.09743 381.527868,155.68352 C399.744224,158.819049 417.734224,163.151949 435.380944,168.654058 C431.331963,186.680673 426.122466,204.426664 419.785029,221.781062 C414.205023,210.55733 408.203506,199.333598 401.720262,188.230335 M127.517179,131.790423 C122.438973,82.3176579 132.816178,46.2973086 155.778503,33.0255968 C163.144699,28.9632474 171.455651,26.9264282 179.864858,27.1225964 C202.967687,27.1225964 232.413257,39.7317265 263.785734,63.9862316 C243.794133,82.7898734 225.448298,103.270812 208.949132,125.204763 C181.761691,128.528025 154.90355,134.14313 128.661281,141.990165 C128.199626,138.556788 127.778115,135.163566 127.456963,131.790423 M98.4529773,182.106474 C101.54406,180.767925 104.695358,179.429376 107.906872,178.090828 C114.220532,204.735668 122.781793,230.7969 133.498624,255.99437 C122.761529,281.241316 114.193296,307.357063 107.8868,334.058539 C56.7434387,313.076786 27.0971497,284.003505 27.0971497,255.99437 C27.0971497,229.450947 53.1907013,202.526037 98.4529773,182.106474 Z M155.778503,478.963143 C132.816178,465.691432 122.438973,429.671082 127.517179,380.198317 C127.838331,376.825174 128.259842,373.431953 128.721497,369.978497 C154.953686,377.878517 181.814655,383.514365 209.009348,386.824134 C225.500295,408.752719 243.832321,429.233234 263.805806,448.042665 C220.069,481.834331 180.105722,492.97775 155.838719,478.963143 M441.502893,380.198317 C446.520883,429.691161 436.203894,465.691432 413.221497,478.963143 C388.974566,493.017906 348.991216,481.834331 305.274481,448.042665 C325.241364,429.232737 343.566681,408.752215 360.050868,386.824134 C387.245915,383.516508 414.107066,377.880622 440.338719,369.978497 C440.820446,373.431953 441.221885,376.825174 441.563109,380.198317 M461.193488,334.018382 C454.869166,307.332523 446.294494,281.231049 435.561592,255.99437 C446.289797,230.744081 454.857778,204.629101 461.173416,177.930202 C512.216417,198.911955 541.942994,227.985236 541.942994,255.99437 C541.942994,284.003505 512.296705,313.076786 461.153344,334.058539"/></g></g></g></svg>
          <span className="ml-2 text-sm">React</span>
        </span>
      ),
      'TailwindCSS': (
        <span title="TailwindCSS" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="https://www.w3.org/2000/svg" width="28" height="28" viewBox="0,0,256,256" style={{fill:'#FFFFFF'}}><g fill="#ffffff" fillRule="nonzero" stroke="none" strokeWidth="1" strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit="10" strokeDasharray="" strokeDashoffset="0" fontFamily="none" fontWeight="none" fontSize="none" textAnchor="none" style={{mixBlendMode: 'normal'}}><g transform="scale(10.66667,10.66667)"><path d="M18,13c-1.858,0 -3.489,-1.297 -5.748,-3.092c-1.521,-1.209 -5.006,-1.619 -5.552,-1.194l-1.407,-1.421c2.053,-2.054 3.293,-3.293 5.707,-3.293c3.018,0 5.214,1.688 6.979,3.046c0.554,0.425 1.077,0.827 1.527,1.092c0.342,0.201 1.073,0.436 1.685,0.253c0.391,-0.115 0.689,-0.39 0.914,-0.838l1.834,0.789c-1.06,2.917 -3.28,4.658 -5.939,4.658zM8.983,6.633c1.707,0.234 3.512,0.913 4.514,1.709c2.135,1.698 3.397,2.658 4.503,2.658c0.745,0 1.435,-0.207 2.042,-0.599c-0.611,-0.097 -1.158,-0.31 -1.548,-0.539c-0.557,-0.328 -1.128,-0.767 -1.732,-1.231c-1.606,-1.234 -3.425,-2.631 -5.762,-2.631c-0.786,0 -1.359,0.176 -2.017,0.633zM13,20c-1.858,0 -3.489,-1.297 -5.748,-3.092c-1.52,-1.208 -5.006,-1.621 -5.552,-1.194l-1.407,-1.421c2.053,-2.054 3.293,-3.293 5.707,-3.293c3.018,0 5.214,1.688 6.979,3.046c0.554,0.425 1.077,0.827 1.527,1.092c0.342,0.201 1.073,0.433 1.685,0.253c0.391,-0.115 0.689,-0.39 0.914,-0.838l1.834,0.789c-1.06,2.917 -3.28,4.658 -5.939,4.658zM3.983,13.633c1.707,0.234 3.512,0.913 4.514,1.709c2.135,1.698 3.397,2.658 4.503,2.658c0.745,0 1.435,-0.207 2.042,-0.599c-0.611,-0.097 -1.158,-0.31 -1.548,-0.539c-0.557,-0.328 -1.128,-0.767 -1.732,-1.231c-1.606,-1.234 -3.425,-2.631 -5.762,-2.631c-0.786,0 -1.359,0.176 -2.017,0.633z"></path></g></g></svg>
          <span className="ml-2 text-sm">TailwindCSS</span>
        </span>
      ),
      'Prisma': (
        <span title="Prisma" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg viewBox="0 0 256 310" width="28" height="28" xmlns="https://www.w3.org/2000/svg"><path fill="#fff" d="M254.313 235.519L148 9.749A17.063 17.063 0 00133.473.037a16.87 16.87 0 00-15.533 8.052L2.633 194.848a17.465 17.465 0 00.193 18.747L59.2 300.896a18.13 18.13 0 0020.363 7.489l163.599-48.392a17.929 17.929 0 0011.26-9.722 17.542 17.542 0 00-.101-14.76l-.008.008zm-23.802 9.683l-138.823 41.05c-4.235 1.26-8.3-2.411-7.419-6.685l49.598-237.484c.927-4.443 7.063-5.147 9.003-1.035l91.814 194.973a6.63 6.63 0 01-4.18 9.18h.007z" /></svg>
          <span className="ml-2 text-sm">Prisma</span>
        </span>
      ),
      'TypeScript': (
        <span title="TypeScript" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="https://www.w3.org/2000/svg" width="28" height="28" viewBox="0,0,256,256" style={{fill:'#FFFFFF'}}><g fill="#ffffff" fillRule="nonzero" stroke="none" strokeWidth="1" strokeLinecap="butt" strokeLinejoin="miter" strokeMiterlimit="10" strokeDasharray="" strokeDashoffset="0" fontFamily="none" fontWeight="none" fontSize="none" textAnchor="none" style={{mixBlendMode: 'normal'}}><g transform="scale(5.12,5.12)"><path d="M45,4h-40c-0.553,0 -1,0.448 -1,1v40c0,0.552 0.447,1 1,1h40c0.553,0 1,-0.448 1,-1v-40c0,-0.552 -0.447,-1 -1,-1zM29,26.445h-5v15.555h-4v-15.555h-5v-3.445h14zM30.121,41.112v-4.158c0,0 2.271,1.712 4.996,1.712c2.725,0 2.62,-1.782 2.62,-2.026c0,-2.586 -7.721,-2.586 -7.721,-8.315c0,-7.791 11.25,-4.717 11.25,-4.717l-0.14,3.704c0,0 -1.887,-1.258 -4.018,-1.258c-2.131,0 -2.9,1.013 -2.9,2.096c0,2.795 7.791,2.516 7.791,8.141c0.001,8.664 -11.878,4.821 -11.878,4.821z"></path></g></g></svg>
          <span className="ml-2 text-sm">TypeScript</span>
        </span>
      ),
      'Python': (
        <span title="Python" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 256 256" style={{fill:'#FFFFFF'}}>
            <g fill="#ffffff">
              <path d="M127.9 16.1c-23.7 0-43 19.3-43 43v21.5h86V59.1c0-23.7-19.3-43-43-43zm-27.5 43c0-15.2 12.3-27.5 27.5-27.5s27.5 12.3 27.5 27.5v21.5h-55V59.1zm-43 43c0-23.7 19.3-43 43-43h21.5v86H59.1c-23.7 0-43-19.3-43-43zm43-27.5c-15.2 0-27.5 12.3-27.5 27.5s12.3 27.5 27.5 27.5h21.5v-55H59.1zm127.8 27.5c0 23.7-19.3 43-43 43h-21.5v-86h21.5c23.7 0 43 19.3 43 43zm-43 27.5c15.2 0 27.5-12.3 27.5-27.5s-12.3-27.5-27.5-27.5h-21.5v55h21.5zm-43 43c0 23.7 19.3 43 43 43h21.5v-86h-21.5c-23.7 0-43 19.3-43 43zm43 27.5c-15.2 0-27.5-12.3-27.5-27.5s12.3-27.5 27.5-27.5h21.5v55h-21.5zm-43-27.5c0-23.7 19.3-43 43-43h21.5v86h-21.5c-23.7 0-43-19.3-43-43zm43-27.5c-15.2 0-27.5 12.3-27.5 27.5s12.3 27.5 27.5 27.5h21.5v-55h-21.5z"/>
            </g>
          </svg>
          <span className="ml-2 text-sm">Python</span>
        </span>
      ),
      'NumPy': (
        <span title="NumPy" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 256 256" style={{fill:'#FFFFFF'}}>
            <g fill="#ffffff">
              <rect width="256" height="256" rx="60"/>
              <path d="M64 64h128v128H64z" fill="#4B8BBE"/>
              <path d="M96 96h64v64H96z" fill="#306998"/>
            </g>
          </svg>
          <span className="ml-2 text-sm">NumPy</span>
        </span>
      ),
      'Three.js': (
        <span title="Three.js" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 256 256" style={{fill:'#FFFFFF'}}>
            <g fill="#ffffff">
              <circle cx="128" cy="128" r="128"/>
              <path d="M128 64l64 112H64z" fill="#000"/>
            </g>
          </svg>
          <span className="ml-2 text-sm">Three.js</span>
        </span>
      ),
      'Ciencia de Datos': (
        <span title="Ciencia de Datos" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 256 256" style={{fill:'#FFFFFF'}}>
            <g fill="#ffffff">
              <rect width="256" height="256" rx="60"/>
              <path d="M64 192h128v-16H64zm0-32h128v-16H64zm0-32h128v-16H64zm0-32h128V80H64z"/>
            </g>
          </svg>
          <span className="ml-2 text-sm">Ciencia de Datos</span>
        </span>
      ),
      'PostgreSQL': (
        <span title="PostgreSQL" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 256 256" style={{fill:'#FFFFFF'}}>
            <g fill="#ffffff">
              <circle cx="128" cy="128" r="100"/>
              <path d="M128 64l64 112H64z"/>
            </g>
          </svg>
          <span className="ml-2 text-sm">PostgreSQL</span>
        </span>
      ),
      'Academia': (
        <span title="Academia" className="flex items-center gap-2 px-3 py-1" style={{backgroundColor: '#ff8200', opacity: 0.8, borderRadius: '1.5rem'}}>
          <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 256 256" style={{fill:'#FFFFFF'}}>
            <g fill="#ffffff">
              <circle cx="128" cy="128" r="100"/>
              <path d="M128 64l64 112H64z"/>
            </g>
          </svg>
          <span className="ml-2 text-sm">Academia</span>
        </span>
      )
    };

    return (
      <div className="flex flex-wrap gap-4 justify-center mt-4">
        {technologies.map((tech, index) => {
          const techElement = techSVGs[tech];
          return techElement ? React.cloneElement(techElement, { key: `${tech}-${index}` }) : null;
        }).filter(Boolean)}
      </div>
    );
  };

  return (
    <section id="proyectos" className="w-full py-20 bg-[#041737] text-white">
      {/* Sección de Startups */}
      <div className="max-w-5xl mx-auto px-4 text-center">
        <h2 className="font-title text-4xl md:text-5xl mb-8 text-[#FF8200]">Proyectos</h2>
        <p className="text-lg md:text-xl mb-12" style={{ fontFamily: 'Be Vietnam, sans-serif' }}>
          Startups con los que genero ingresos, aplicando tecnología, estrategia y creatividad para resolver problemas reales y construir negocios escalables.
        </p>
        <div className="flex flex-wrap justify-center gap-8 items-start">
          {startupProjects.map((project) => (
            <ProjectCard
              key={project.id}
              logo={project.logo}
              name={project.name}
              github={project.github}
              live={project.live}
              description={project.description}
            >
              {renderTechBadges(project.technologies)}
            </ProjectCard>
          ))}
        </div>
      </div>

      {/* Sección de Trabajo Científico y Tecnológico */}
      <div className="mt-16">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <h3 className="font-title text-3xl md:text-4xl mb-4 text-[#FF8200]">
            Trabajo Científico y Tecnológico
          </h3>
          <p className="text-lg md:text-xl mb-12" style={{ fontFamily: 'Be Vietnam, sans-serif' }}>
            Proyectos donde aplico ciencia, lógica y tecnología para entender problemas complejos y crear soluciones reales.
          </p>
          <div className="flex flex-wrap justify-center gap-8 items-start">
            {scientificProjects.map((project) => (
              <ProjectCard
                key={project.id}
                logo={project.logo}
                name={project.name}
                github={project.github}
                live={project.live}
                description={project.description}
              >
                {renderTechBadges(project.technologies)}
              </ProjectCard>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
};